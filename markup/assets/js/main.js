...
function preparingHintsList(searchString, list){
    ...
    clearHints(list);
    console.log(_searchString.length);
...
function blurOfInputFilter(input, range, slider, minmax = false){
    ...
    if ( input.value.length === 0 || !changeValue ){
        ...
        if (minmax === false){
            ...
            //console.log(currentValue, range[0]);
...
if ( sliderTimeFrame ){
    initRangeSlider(sliderTimeFrame, '#timeframe',  'suffix', ' дней')
}
...
const filterBoxes = document.querySelectorAll('.combo-select');

if ( filterBoxes.length ){

    filterBoxes.forEach( ms => {

        let filterCurrentValue = ms.querySelector('.combo-select__current-value').innerHTML;
        ...
        ms.addEventListener('click', function(){
            console.log(filterCurrentValue);
